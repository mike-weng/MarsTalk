...
-(id)initWithOrigin:(CGPoint)origin andDiameter:(CGFloat)diameter andDelegate:(UIViewController *)controller themeColor:(UIColor *)themeColor mainMenuImage:(UIImage *)image{
...
//    [self.mainView addSubview:cross];
    ...
    CGFloat imageWidth = ((self.mainView.frame.size.width / 2) *sin(M_PI_4) * 2 ) - 14;
    UIImageView *menuImage = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, imageWidth, imageWidth)];
    menuImage.center = CGPointMake(self.mainView.frame.size.width/2, self.mainView.frame.size.height/2);
    menuImage.image = self.mainMenuImage;
    [self.mainView addSubview:menuImage];
...
-(void)setUpSomeDatas{
    ...
    for (int i = 0; i < menuCount; i++) {
        ...
        //设置每个item的图片
        CGFloat imageWidth = ((item.frame.size.width / 2) *sin(M_PI_4) * 2 ) - 13;
        UIImageView *menuImage = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, imageWidth, imageWidth)];
    ...
    [UIView animateWithDuration:kYALShowExtraTabBarItemViewAnimationParameters.duration
         ...
         usingSpringWithDamping:kYALShowExtraTabBarItemViewAnimationParameters.damping
          initialSpringVelocity:kYALShowExtraTabBarItemViewAnimationParameters.velocity
                     ...
                     animations:^{
                         self.mainView.center = CGPointMake(CGRectGetWidth(self.mainView.frame) / 2.f + self.offsetForGooeyMenu, self.mainView.center.y);
                     } completion:NULL];
    ...
    [UIView animateWithDuration:kYALShowExtraTabBarItemViewAnimationParameters.duration
         ...
         usingSpringWithDamping:kYALShowExtraTabBarItemViewAnimationParameters.damping
          initialSpringVelocity:kYALShowExtraTabBarItemViewAnimationParameters.velocity
                        options:kYALShowExtraTabBarItemViewAnimationParameters.options
                     animations:^{
                         self.mainView.center = CGPointMake(CGRectGetWidth(self.containerView.frame) - CGRectGetWidth(self.mainView.frame) / 2.f - self.offsetForGooeyMenu, self.mainView.center.y);
                     } completion:NULL];
...
- (void)hideLeftGooeyMenu {
    [UIView animateWithDuration:kYALHideExtraTabBarItemViewAnimationParameters.duration
                     animations:^{
                         self.mainView.center = CGPointMake( - CGRectGetWidth(self.mainView.frame) / 2.f, self.mainView.center.y);
                     }];
    self.hidden = true;
    self.userInteractionEnabled = false;
}

- (void)hideRightGooeyMenu {
    [UIView animateWithDuration:kYALHideExtraTabBarItemViewAnimationParameters.duration
                     animations:^{
                         self.mainView.center = CGPointMake(self.mainView.center.x + CGRectGetWidth(self.mainView.frame) + self.offsetForGooeyMenu, self.mainView.center.y);
                     }];
    self.hidden = true;
    self.userInteractionEnabled = false;
}
